...
accetylationDat <- function(ba9_81.filepath, ba41_66.filepath, baVermis_62.filepath, samplefilePath, chrFile, binSize, overlapCutoff, outputPath){
  ...
  while(chrInd <=1 ){
    ...
    #while (wStart < 15500){
      ...
      rStart <- dat[,2] - binSize*overlapCutoff
      rEnd <- dat[,3] + binSize*overlapCutoff
      ...
      if(nrow(temp.ba9_81) > 1 || nrow(temp.ba41_66) > 1 || nrow(temp.baVermis) > 1){
      ...
      message(paste0(wEnd," "), appendLF=FALSE)
      wStart <- wEnd + 1
      binID <- binID + 1
    }
