...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=566):
        ...
        self.layer3 = self._make_layer(block, 720, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 1080, num_blocks[3], stride=2)
        self.linear = nn.Sequential(nn.Linear(1080*block.expansion*31, num_classes)
                                    ,nn.Sigmoid())
