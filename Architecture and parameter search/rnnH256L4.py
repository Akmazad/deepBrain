...
class config():
  def __init__(self):
    self.name = "deepbrainStaticConvnet_GCP"
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=566):
        ...
        self.conv1 = nn.Conv1d(4, 256, kernel_size=8, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm1d(256)
        self.layer1 = self._make_layer(block, 320, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 480, num_blocks[1], stride=1)
        self.layer3 = self._make_layer(block, 960, num_blocks[2], stride=1)
        self.linear = nn.Sequential(nn.Linear(960*block.expansion*248, num_classes)
                                    ,nn.Sigmoid())
...
def main():
    ...
    model = RNN(args.SEQ_LEN, 256, 4, args.NUM_OUTPUTS).to(device)
