...
parser.add_argument('--NUM_OUTPUTS', type=int, default=919)
...
class LoadDataset(tdata.Dataset):
    def __init__(self, args, dataPath, dataFile, dataType):
        ...
        if dataType == 'trainData':
            h5f = h5py.File(os.path.join(dataPath, dataFile), 'r')
            self.inputs = h5f['trainxdata']
            self.labels = h5f['traindata']
        elif dataType == 'validData':
            vmat = scipy.io.loadmat(os.path.join(dataPath, dataFile))
            self.inputs = vmat['validxdata']
            self.labels = vmat['validdata']
        else:
            tmat = scipy.io.loadmat(os.path.join(dataPath, dataFile))
            self.inputs = tmat['testxdata']
            self.labels = tmat['testdata']
...
def getAUCscore(predicted, target, args, logger):
    ...
    return np.median(aucs[125:125 + 690]), np.median(aucs[:125]), np.median(aucs[125 + 690:125 + 690 + 104])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    trainDataset = LoadDataset(args, dataPath=DataPath, dataFile=args.TrainingDataFile, dataType="trainData")
    # define sampler
    if args.distributed:
    ...
    valDataset = LoadDataset(args, dataPath=DataPath, dataFile=args.TestingDataFile,  dataType="testData")
