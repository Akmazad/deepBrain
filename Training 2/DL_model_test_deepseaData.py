...
def train(train_loader, model, criterion, optimizer, epoch, args, logger):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # logger.info("Epoch: %d, Batch: %d/%d, Loss: %.3f, acc: %.3f" % (epoch + 1, i, len(train_loader) - 1,
        #                                                                 loss, acc))
        # logger.info("Epoch: %d, Batch: %d/%d, Loss: %.3f, custom_accuracy_metric: %.3f, ACC marks: %.3f, RNA-seq: %.3f, TFs: %.3f" % (epoch + 1, i, len(train_loader) - 1,
        #                                                                 loss, acc, aucs[0], aucs[1], aucs[2]))
        logger.info(
