...
def split(filename, args, logger):
    ...
    makeNumpyArrAlCombination(filename+"_trainingData_TF_filtered_" + chrName, train_df, logger)
    logger.info("Ended")
    del train_df
    ...
    makeNumpyArrAlCombination(filename + "_validationData_TF_filtered_" + chrName, val_df, logger)
    logger.info("Ended")
    del val_df
...
def makeNumpyArrAlCombination(fileName, df, logger):
    ...
    df = df[(df.iloc[:, 9:].T != 0).any()]  # remove rows where there is no TF signals
